          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500 dark:text-zinc-400">
                {isAr ? 'اختر الجامعة واضغط على السهم لعرض كلياتها واستيراد المواد ولائحة التقديرات' : 'Select university, expand to view colleges and restore subjects & grading scale'}
              </p>
            </div>
          </div>
          <button 
            ...
            <div className="space-y-4">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 rtl:pl-4 rtl:pr-10 pr-4 py-3 rounded-2xl bg-zinc-50 dark:bg-zinc-800/60 border border-zinc-200 dark:border-zinc-700 text-xs sm:text-sm font-bold text-zinc-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500"
                />
              </div>
              ...
              <div className="space-y-3 max-h-[420px] overflow-y-auto pr-1">
                {filteredUniversities.map((group) => {
                  ...
                  return (
                    <div 
                      key={group.key}
                      className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-2xs overflow-hidden transition-all"
                    >
                      ...
                      {isExpanded && (
                        <div className="p-3 bg-zinc-50/70 dark:bg-zinc-800/30 border-t border-zinc-100 dark:border-zinc-800/80 space-y-2 animate-in fade-in duration-150">
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                            {group.databases.map((dbItem) => (
                              <div
                                key={dbItem.id}
                                onClick={() => setSelectedDb(dbItem)}
                                className="p-3.5 rounded-2xl border border-zinc-200 dark:border-zinc-700/70 bg-white dark:bg-zinc-900 hover:border-indigo-500 dark:hover:border-indigo-500 text-left rtl:text-right transition-all group flex flex-col justify-between gap-2.5 shadow-2xs hover:shadow-md cursor-pointer"
                              >
                                <div className="space-y-1 min-w-0">
                                  <div className="flex items-center justify-between gap-2">
                                    <div className="flex items-center gap-2 min-w-0">
                                      <GraduationCap size={16} className="text-indigo-600 dark:text-indigo-400 shrink-0" />
                                      <h5 className="font-extrabold text-xs sm:text-sm text-zinc-900 dark:text-white truncate">
                                        {dbItem.collegeNameAr || dbItem.collegeNameEn}
                                      </h5>
                                    </div>
                                    <ArrowIcon size={14} className="text-zinc-400 group-hover:text-indigo-600 transition-transform group-hover:scale-110 shrink-0" />
                                  </div>
                                ...
                                <div className="flex flex-wrap items-center gap-1.5 pt-1.5 border-t border-zinc-100 dark:border-zinc-800 text-[10px] text-zinc-500 dark:text-zinc-400">
                                  <span className="bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 font-bold px-2 py-0.5 rounded-md">
                                  ...
                                  <span className="bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 rounded-md">
                                    {dbItem.driveFiles?.length || 0} {isAr ? 'درايف' : 'files'}
                                  </span>
                                  {dbItem.gradingScale && dbItem.gradingScale.length > 0 && (
                                    <span className="bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 font-bold px-2 py-0.5 rounded-md">
